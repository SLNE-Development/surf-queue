...
fun CommandTree.queueTickCommand() = literalArgument("tick") {
    withPermission(SurfQueuePermissions.COMMAND_TICK)
    ...
    anyExecutorSuspend { sender, _ ->
        ...
        val tickBefore = queue.getTickCount()
        queue.tickSecond()
        val tickAfter = queue.getTickCount()

        sender.sendText {
            appendSuccessPrefix()
            success("Manually ticked queue. Tick count: ")
            variableValue("$tickBefore")
            success(" → ")
            variableValue("$tickAfter")
        }
    }
}
